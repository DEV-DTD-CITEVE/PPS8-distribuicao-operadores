...
export const normalizarHistorico = (historico: any[]): HistoricoBalanceamento[] => {
...
export const obterHistorico = (): HistoricoBalanceamento[] => {
  try {
    ...
    if (!data) return [];
    return normalizarHistorico(JSON.parse(data));
  } catch (error) {
...
export const removerItemHistorico = (id: string): void => {
  try {
    ...
    console.error("Erro ao remover item do historico:", error);
  }
};

export const calcularEstatisticasHistorico = (historicoInput: HistoricoBalanceamento[]) => {
  const historico = normalizarHistorico(historicoInput as any[]);
  ...
  if (historico.length === 0) {
    return {
      ...
      oleMedio: 0,
      perdasMedia: 0,
      melhorConfiguracao: null as HistoricoBalanceamento | null,
      taktTimeMedio: 0,
    };
  }
...
export const obterEstatisticas = () => calcularEstatisticasHistorico(obterHistorico());

export const criarHistoricoDemo = (): HistoricoBalanceamento[] => {
  ...
  const produtosInfo = {
    GRUPO001: { nome: "Calcas", ref: "GA-CALCAS" },
  ...
  return historicoDemo;
};

export const gerarHistoricoDemo = (): void => {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(criarHistoricoDemo()));
  } catch (error) {
    console.error("Erro ao gerar historico demo:", error);
  }
};
