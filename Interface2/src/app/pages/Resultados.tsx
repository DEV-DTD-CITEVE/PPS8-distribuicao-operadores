...
export default function Resultados() {
  ...
  // Tentar obter dados da seguinte prioridade:
  // 1. Navigation state (mais recente)
  // 2. Persistent storage (resultadosAtuais)
  // 3. sessionStorage (fallback)
  let dataSource = location.state;
  
  if (!dataSource || !dataSource.resultados) {
    dataSource = dados.resultadosAtuais;
  }
  ...
  const { resultados, operadores, operacoes, config, atribuicoesManual } = dataSource as {
  ...
  const warnings = resultados.avisos || [];
