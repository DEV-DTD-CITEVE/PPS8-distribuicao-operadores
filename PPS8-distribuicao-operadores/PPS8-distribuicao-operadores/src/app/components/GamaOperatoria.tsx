...
export function GamaOperatoria({
  ...
  const handleToggleCritica = async (operacao: Operacao) => {
  if (!onToggleCritica || criticalSavingId) return;

  const nextCritical = !Boolean(operacao.critica);

  setCriticalSavingId(operacao.id);
          ...
          <DialogParts.Dialog open={showDialog} onOpenChange={setShowDialog}>
            ...
            <DialogParts.DialogContent className="rounded-sm">
              <DialogParts.DialogHeader>
                <DialogParts.DialogTitle className="text-base font-semibold">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <Label htmlFor="tempo" className="text-xs font-medium">
                ...
                <div>
                  <Label htmlFor="tipoMaquina" className="text-xs font-medium">
                    Máquina (opcional)
                  </Label>
                  <Input
                    id="tipoMaquina"
                ...
                <div className="flex items-center space-x-2">
                  <CheckboxParts.Checkbox
                    ...
                    onCheckedChange={(checked) =>
                      setNovaOperacao({
                        ...novaOperacao,
                        permitirAgrupamento: !!checked,
                      })
                    }
                  />
                  <Label
                    htmlFor="permitirAgrupamento"
          ...
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="p-3 text-left text-xs font-semibold text-gray-600 uppercase w-24">
                  Crítica
                </th>
            ...
            <tbody>
              {operacoes.map((operacao, index) => (
                ...
                >
                  ...
                  <td className="p-3 font-mono text-sm text-gray-700">
                    {operacao.tempo ? operacao.tempo.toFixed(2) : "0.00"}
                  </td>
                  <td className="p-3 text-sm text-gray-600">
                    {operacao.tipoMaquina || "—"}
                  </td>
                  <td className="p-3 text-sm text-gray-600">
                    {operacao.largura ? operacao.largura.toFixed(0) : "0"}
                  </td>
                  <td className="p-3 text-sm text-gray-600">
                    {operacao.ponto || "—"}
                  </td>
                  ...
                  <td className="p-3 text-center">
                    <CheckboxParts.Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        void handleToggleCritica(operacao)
                      }
                      className="text-xs"
                    />
                  </td>
